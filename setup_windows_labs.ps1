...
function Install-Winget($id, $name){
  if (winget list --id $id --exact 2>$null | Select-String -Quiet $id) { Ok "$name already installed"; return }
...
for m in mods:
    ...
    except Exception as e: bad.append(m); print(f"  [FAIL] {m}: {e}")
from pyspark.sql import SparkSession
spark = SparkSession.builder.master("local[1]").config("spark.ui.enabled","false").getOrCreate()
df = spark.createDataFrame([(1,"a"),(2,"b")],["id","v"]); assert df.count()==2
import tempfile, os; p = os.path.join(tempfile.mkdtemp(), "t"); df.write.mode("overwrite").parquet(p); print("  [OK] PySpark local + parquet write (winutils working)")
spark.stop()
sys.exit(1 if bad else 0)
PYEOF
if ($LASTEXITCODE -ne 0) { Warn "Some Python imports failed — see above." } else { Ok "Python stack verified" }
foreach ($c in @("git --version","java -version","az version","databricks --version")) { try { Invoke-Expression "$c 2>&1 | Select-Object -First 1" | ForEach-Object { Ok "$c -> $_" } } catch { Warn "$c failed" } }

# ---------- 7. Optional smoke test of one lab
if ($RunSmokeTest) {
  Step "Smoke test: L12 solution notebook"
  $nb = Join-Path $LabRoot "LLMOps_Advanced_Labs\labs\L12\L12_solution.ipynb"
  if (Test-Path $nb) {
    $env:LAB_MODE = "SMOKE"; $env:DATA_DIR = Join-Path $LabRoot "LLMOps_Advanced_Labs\data"
    & $py -c "import nbformat,os; from nbclient import NotebookClient; p=r'$nb'; nb=nbformat.read(p,as_version=4); NotebookClient(nb,timeout=600,kernel_name='llmops',resources={'metadata':{'path':os.path.dirname(p)}}).execute(); print('  [OK] L12 executed')"
  } else { Warn "Lab package not found at $LabRoot\LLMOps_Advanced_Labs — unzip it there and re-run with -RunSmokeTest." }
}

Step "Done"
Write-Host @"
Next steps:
  1. Unzip LLMOps_Advanced_Labs.zip into $LabRoot (if not already).
